...
const fs=require("fs");
const SRC=fs.readFileSync(_W.FILE,"utf8");
...
const SQLDIR=_W.dir;   /* the repo root, not tests/ — see _where.js */
const sqlOrder=fs.readdirSync(SQLDIR).filter(x=>x.endsWith(".sql"))
  .sort((a,b)=>(a==="supabase-schema.sql"?0:1)-(b==="supabase-schema.sql"?0:1)
              ||a.localeCompare(b));
const sqlAllowed=()=>{
  let last=null;
  for(const f of sqlOrder){
    const t=fs.readFileSync(SQLDIR+"/"+f,"utf8");
