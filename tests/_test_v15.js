...
/* Slice to the END OF THE TABLE, not to the next function. The old boundary
   swept up whatever happened to be declared in between — when the collection
   and CSV code landed there, their column lists were counted as menu tiles. */
const GRP_START=SRC.indexOf("const OTHER_GROUPS=[");
const GRP_SRC=SRC.slice(GRP_START,SRC.indexOf("\n];",GRP_START));
