...
ok(await p.evaluate(()=>{
    ...
    return css.some(r=>r.conditionText&&r.conditionText.includes("print")
      &&/\.foot\{display:none/.test(r.cssText.replace(/\s/g,"")))}),
   "a print rule hides the footer, so a pull sheet is a shopping list and nothing else");
ok(await p.evaluate(()=>!!document.querySelector(".foot .fdis")),
   "…while the site itself still carries it — it's a Community Code condition, not decoration");
